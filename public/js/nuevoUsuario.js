//public/js/nuevoUsuario.js
// Clase Usuario para representar un usuario
class Usuario {
    constructor(nombre, apellido, fechaNacimiento, email, contrasena, pregunta, respuesta, idrol) {
        this.nombre = nombre;
...
async function handleFormSubmit(event) {
    ...
    let idrol = parseInt(document.querySelector('select[name="idrol"]').value);
    ...
    let nuevoUsuario = new Usuario(nombre, apellido, fechaNacimiento, email, contrasena, pregunta, respuesta, idrol); // Asegúrate de incluir idrol

    // Enviar el nuevo usuario a la base de datos
    try {
        const registerResponse = await fetch('/usuarios/', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
